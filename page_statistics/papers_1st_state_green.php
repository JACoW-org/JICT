...
//$editors =file_read_json( "../data/editors.json", true );
$editors =$Indico->data['editors'];

$content =false;
foreach ($editors as $e) {
    foreach ($e['papers']['g'] as $pid) {
        $content .=get_paper_revisions( $pid );
    }
}
...
function get_paper_revisions( $_pid ) {
    ...
    foreach ($pedit['revisions'] as $r_id =>$r) {
        ...
        $keys =[ 'created_dt', 'reviewed_dt', 'comment', 'tags', '#count.files', '#count.tags', 'editor.full_name', 'submitter.full_name', 'initial_state.name', 'final_state.name' ];
        ...
        foreach ($keys as $k) {
            ...
            if (empty($k2)) {
                if ($k == 'tags') {
                    $tags =false;
                    foreach ($r['tags'] as $t) {
                        $tags .="<span class='tag' style='background:$t[color]'>$t[code]</span>";
                    }
                    $row[$k] =$tags;
                } else if (substr( $k, -3 ) == '_dt') $row[$k] =str_replace( "T", "<br />", substr( $r[$k], 5, 14 ));
                else $row[$k] =$r[$k];
            }
            else if ($k1 == '#count') $row[$k] =count($r[$k2]);
            else $row[$k] =$r[$k1][$k2];
        }
